...
def draw_chessboard_plane(ax, rmat, tvec, chessboard_size, square_size, distance, color_map, norm):
    """绘制标定板的平面，并根据距离相机的远近设置渐变色"""
...
def visualize_camera_and_chessboard(rotation_matrices, translation_vectors, chessboard_size, square_size, mtx, image_width, image_height):
    """可视化相机和标定板的位置关系，模拟相机视场内的拍摄角点"""
    ...
    for i, (rmat, tvec) in enumerate(zip(rotation_matrices, translation_vectors)):
        ...
        # 绘制标定板平面
        draw_chessboard_plane(ax, rmat, tvec, chessboard_size, square_size, distance, color_map, norm)
...
config = {
    ...
    "chessboard_size": (33, 16),  # 棋盘格的内角点数
    "square_size": 5.0,  # 格子大小，单位毫米
}

# 加载标定数据
mtx, dist, rotation_matrices, translation_vectors = load_calibration_data(
    config["output_dir"]
)
...
visualize_camera_and_chessboard(
    rotation_matrices, translation_vectors, config["chessboard_size"], config["square_size"], mtx, image_width, image_height
)
